...
interface TratamientoFormProps {
  onSubmit: (tratamiento: Tratamiento) => Promise<void>; // Asegúrate de que onSubmit sea asíncrono
...
const TratamientoForm: React.FC<TratamientoFormProps> = ({
  ...
  const onFormSubmit = async (data: TratamientoFormValues) => {
    setLoading(true); // Activa el spinner
    try {
      await onSubmit(data); // Espera a que la función onSubmit (la que hace la llamada a la API) termine
      reset();
    } catch (e) {
      console.error(e);
    } finally {
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm">
      ...
      <form onSubmit={handleSubmit(onFormSubmit)} className="p-4">
        {/* ... (el resto de tu formulario es igual) ... */}
